...
int main(int argc, char** argv)
{
  ...
  n_steps = 75.0;     /* NO OF TIMESTEPS PER BINARY ORBIT */
  out_steps = 15000.0;   /* NO OF TIMESTEPS BETWEEN OUTPUT DUMPS */
  ...
  sprintf(root,"res3body_q=%.3f_e=%.3f_a=%.3f",massrat,eccen,r_planet);
  printf("%s\n",root);

  /* OPEN OUTPUT FILES */
  sprintf(datafile,"data_rep_1/planet_%s.txt",root);
  ...
  /* CREATE UNITS FILE */
  sprintf(unitsfile,"data_rep_10/%s.txt",root);  
